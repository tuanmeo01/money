...
const Account: FC = (): ReactElement => {
    ...
    return (
        <ContentWrapper>
            <Row gutter={32}>
                <Col span={5}>
                    <ContentTitle title={"Thông tin tài khoản"} titleLevel={4} icon={<UserOutlined />} />
                    <AccountLink link={ACCOUNT_USER_INFO} title={"Thông tin cá nhân"} />
                    {isAdmin ? (
                        <>
                            <AccountLink link={ACCOUNT_ADMIN_ADD} title={"Thêm sản phẩm"} />
                            <AccountLink link={ACCOUNT_ADMIN_PERFUMES} title={"Danh sách sản phẩm"} />
                            <AccountLink link={ACCOUNT_ADMIN_ORDERS} title={"Danh sách order"} />
                            <AccountLink link={ACCOUNT_ADMIN_USERS} title={"Danh sách người dùng"} />
                        </>
                    ) : (
                        <>
                            <AccountLink link={ACCOUNT_USER_EDIT} title={"Đổi mật khẩu"} />
                            <AccountLink link={ACCOUNT_USER_ORDERS} title={"Danh sách order"} />
                        </>
                    )}
                </Col>
                <Col span={19}>
